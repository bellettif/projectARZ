'''
Created on Oct 8, 2014

@author: cusgadmin
'''

import numpy as np
from matplotlib import pyplot as plt
import cPickle as pickle

from time_domain_responses import *
from fourier_transform import compute_input_fft
from fourier_transform import compute_inv_fft

for n_grid in [80, 100, 120]:
    params = pickle.load(open('../system_params/%d_%d_params.pi' % (n_grid, n_grid), 'rb'))
    #
    lambda_1 = params['lambda_1']
    lambda_2 = params['lambda_2']
    rho_star = params['rho_star']
    v_star = params['v_star']
    q_star = params['q_star']
    #
    print 'N_grid = %d' % n_grid
    print 'Lambda_1 = %.2f' % lambda_1
    print 'Lambda_2 = %.2f' % lambda_2
    print 'rho_star = %.2f' % rho_star
    print 'v_star = %.2f' % v_star
    print 'q_star = %.2f' % q_star
    #
    mat_dict = pickle.load(open('../matrices/mat_dict_%d_%d.pi' % (n_grid, n_grid), 'rb'))
    #
    rho_data = mat_dict['rho'] - rho_star
    q_data = mat_dict['q'] - q_star
    v_data = mat_dict['v'] - v_star
    dx = mat_dict['dx']
    dt = mat_dict['dt']
    n_grid_x = q_data.shape[0]
    n_grid_t = q_data.shape[1]
    #
    x_values = np.arange(n_grid_x) * dx
    t_values = np.arange(n_grid_t) * dt
    #
    T = t_values[-1]
    L = x_values[-1]
    #
    xi_1_data = (rho_star * lambda_2 / (lambda_1 - lambda_2)) * v_data + q_data
    xi_2_data = (rho_star * lambda_1 / (lambda_1 - lambda_2)) * v_data
    #
    #    Downstream boundary condition
    #
    xi_1_data_0 = xi_1_data[0]
    xi_2_data_0 = xi_2_data[0]
    #
    #    Upstream boundary condition
    #
    xi_1_data_L = xi_1_data[-1]
    xi_2_data_L = xi_2_data[-1]
    #
    mean_0, modules_0, args_0, freqs_0 = compute_input_fft(xi_1_data_0, dt)
    mean_L, modules_L, args_L, freqs_L = compute_input_fft(xi_2_data_L, dt)
    #
    reconstruct_xi_1_data = compute_inv_fft(mean_0,
                                         modules_0,
                                         args_0,
                                         freqs_0, 
                                         dt,
                                         len(xi_1_data_0)) 
    reconstruct_xi_2_data = compute_inv_fft(mean_L,
                                         modules_L,
                                         args_L,
                                         freqs_L, 
                                         dt,
                                         len(xi_2_data_L))
    plt.subplot(211)
    plt.plot(xi_1_data_0)
    plt.plot(reconstruct_xi_1_data)
    plt.title('xi_1(0,t) condition n = %d' % n_grid)
    plt.xlabel('t')
    plt.ylabel('xi_1')
    plt.legend(('xi_1', 'FFT inv'), 'upper right')
    plt.subplot(212)
    plt.plot(xi_2_data_L)
    plt.plot(reconstruct_xi_2_data)
    plt.title('xi_2(L,t) condition n = %d' % n_grid)
    plt.xlabel('t')
    plt.ylabel('xi_2')
    plt.legend(('xi_2', 'FFT inv'), 'upper right')
    plt.savefig('plots/Boundary_conditions_FFT_n_%d.png' % n_grid)
    plt.close()
    #    
    for TAU in [15.0, 20.0, 25.0, 30.0]:
        print '\ttau = %.2f' % TAU
        #
        xi_1_sim = np.zeros((n_grid_x, n_grid_t))
        xi_2_sim = np.zeros((n_grid_x, n_grid_t))
        #
        for i in range(len(modules_0))[1:]:
            xi_1_sim += modules_0[i] * \
                        np.asanyarray([[fund_cos_1_1(2 * np.pi * freqs_0[i], t + args_0[i] / (2 * np.pi * freqs_0[i]),
                                                   x, lambda_1, lambda_2, TAU, L)
                                          for t in t_values]
                                         for x in x_values])
            xi_2_sim += modules_0[i] * \
                        np.asanyarray([[fund_cos_1_2(2 * np.pi * freqs_0[i], t + args_0[i] / (2 * np.pi * freqs_0[i]),
                                                   x, lambda_1, lambda_2, TAU, L)
                                          for t in t_values]
                                         for x in x_values])
        
        for i in range(len(modules_L))[1:]:
            xi_1_sim += modules_L[i] * \
                        np.asanyarray([[fund_cos_2_1(2 * np.pi * freqs_L[i], t + args_L[i] / (2 * np.pi * freqs_L[i]),
                                                   x, lambda_1, lambda_2, TAU, L)
                                          for t in t_values]
                                         for x in x_values])
            xi_2_sim += modules_L[i] * \
                        np.asanyarray([[fund_cos_2_2(2 * np.pi * freqs_L[i], t + args_L[i] / (2 * np.pi * freqs_L[i]),
                                                   x, lambda_1, lambda_2, TAU, L)
                                          for t in t_values]
                                         for x in x_values])
        #
        xi_1_sim /= float(len(freqs_0))
        xi_2_sim /= float(len(freqs_L))
        #
        xi_1_sim += mean_0 * np.asanyarray([[fund_H_1_1(t, x, lambda_1, lambda_2, TAU, L)
                                          for t in t_values]
                                         for x in x_values])
        xi_1_sim += mean_L * np.asanyarray([[fund_H_2_1(t, x, lambda_1, lambda_2, TAU, L)
                                          for t in t_values]
                                         for x in x_values])
        
        xi_2_sim += mean_0 * np.asanyarray([[fund_H_1_2(t, x, lambda_1, lambda_2, TAU, L)
                                          for t in t_values]
                                         for x in x_values])[-1]
        
        xi_2_sim += mean_L * np.asanyarray([[fund_H_2_2(t, x, lambda_1, lambda_2, TAU, L)
                                          for t in t_values]
                                         for x in x_values])
        #
        q_sim = xi_1_sim - lambda_2 / lambda_1 * xi_2_sim
        v_sim = (lambda_1 - lambda_2) / (rho_star * lambda_1) * xi_2_sim
        #
        #    Check that boundary conditions do match
        #
        plt.subplot(211)
        plt.plot(xi_1_data_0)
        plt.plot(xi_1_sim[0])
        plt.title('xi_1(0,t) condition n = %d' % n_grid)
        plt.xlabel('t')
        plt.ylabel('xi_1')
        plt.legend(('xi_1_data', 'xi_1_sim'), 'upper right')
        plt.subplot(212)
        plt.plot(xi_2_data_L)
        plt.plot(xi_2_sim[-1])
        plt.title('xi_2(L,t) condition n = %d' % n_grid)
        plt.xlabel('t')
        plt.ylabel('xi_2')
        plt.legend(('xi_2', 'xi_2_sim'), 'upper right')
        plt.savefig('plots/Boundary_conditions_check_n=%d_tau=%.2f.png' % (n_grid, TAU))
        plt.close()
        #
        #    Check maps in xi_1, xi_2 domain
        #
        #    Xi_1
        #        Values from data
        plt.subplot(231)
        plt.imshow(xi_1_data[::-1], interpolation = 'None')
        plt.title('xi_1 data')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #        Simulated values
        plt.subplot(232)
        plt.imshow(xi_1_sim[::-1], interpolation = 'None')
        plt.title('xi_1 sim')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #        Error
        plt.subplot(233)
        plt.imshow(xi_1_data - xi_1_sim[::-1], interpolation = 'None')
        plt.title('xi_1 data - xi_1 sim')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #    Xi_2
        #        Values from data
        plt.subplot(234)
        plt.imshow(xi_2_data[::-1], interpolation = 'None')
        plt.title('xi_2 data')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #        Simulated values
        plt.subplot(235)
        plt.imshow(xi_2_sim[::-1], interpolation = 'None')
        plt.title('xi_2 sim')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #        Error
        plt.subplot(236)
        plt.imshow(xi_2_data[::-1] - xi_2_sim[::-1], interpolation = 'None')
        plt.title('xi_2 data - xi_2 sim')
        plt.xlabel('t')
        plt.ylabel('x')   
        plt.colorbar()
        # Done
        fig = plt.gcf()
        fig.set_size_inches((12, 6))
        plt.savefig('plots/xi_map_n=%d_tau=%.2f.png' % (n_grid, TAU))
        plt.close()
        #
        #    Plot histogram of error in xi_1, xi_2 domain
        #
        plt.hist(np.ravel(xi_1_data[::-1] - xi_1_sim[::-1]), bins = 100)
        plt.xlabel('xi_1 error')
        plt.title('Histogram of xi_1 error')
        plt.ylabel('xi_1 data - xi_1 sim')
        plt.savefig('plots/xi_1_error_%d_%.2f.png' % (n_grid, TAU))
        plt.close()
        #
        plt.hist(np.ravel(xi_2_data[::-1] - xi_2_sim[::-1]), bins = 100)
        plt.xlabel('xi_2 error')
        plt.title('Histogram of xi_2 error')
        plt.ylabel('xi_2 data - xi_2 sim')
        plt.savefig('plots/xi_2_error_%d_%.2f.png' % (n_grid, TAU))
        plt.close()
        #
        #    Check maps in v, q domain
        #
        #    v
        #        Values from data
        plt.subplot(231)
        plt.imshow(v_data[::-1], interpolation = 'None')
        plt.title('v data')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #        Simulated values
        plt.subplot(232)
        plt.imshow(v_sim[::-1], interpolation = 'None')
        plt.title('v sim')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #        Error
        plt.subplot(233)
        plt.imshow(v_data[::-1] - v_sim[::-1], interpolation = 'None')
        plt.title('v data - v sim')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #    q
        #        Values from data
        plt.subplot(234)
        plt.imshow(q_data[::-1], interpolation = 'None')
        plt.title('q data')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #        Simulated values
        plt.subplot(235)
        plt.imshow(q_sim[::-1], interpolation = 'None')
        plt.title('q sim')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        #        Error
        plt.subplot(236)
        plt.imshow(q_data[::-1] - q_sim[::-1], interpolation = 'None')
        plt.title('q data - q sim')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.colorbar()
        # Done
        plt.savefig('plots/vq_map_n=%d_tau=%.2f.png' % (n_grid, TAU))
        plt.close()
        #
        #    Plot histogram of error in v, q domain
        #
        plt.hist(np.ravel(v_data[::-1] - v_sim[::-1]), bins = 100)
        plt.xlabel('v error')
        plt.title('Histogram of v error')
        plt.ylabel('v data - v sim')
        plt.savefig('plots/v_error_%d_%.2f.png' % (n_grid, TAU))
        plt.close()
        #
        plt.hist(np.ravel(q_data[::-1] - q_sim[::-1]), bins = 100)
        plt.xlabel('q error')
        plt.title('Histogram of q error')
        plt.ylabel('q data - q sim')
        plt.savefig('plots/q_error_%d_%.2f.png' % (n_grid, TAU))
        plt.close()
