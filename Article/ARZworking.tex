...
\begin{itemize}
\item In $\psi_{11}$, $e^{-\frac{sx}{\lambda_{1}}}$ is a distributed delay with propagation speed $\lambda_{1}$ and $e^{-\frac{x}{\tau\lambda_{1}}}$ a distributed gain with characteristic distance $\tau\lambda_{1}$.
...
Interpreting the approximate transfer functions in low frequencies:
\begin{itemize}
...
\item We can almost draw the same conclusions for $\theta_{21}$ where a derivation term is combined to the transfer function.
\item Once simplified, $theta_{22}$ corresponds to a distributed delay with propagation speed $\lambda_{2}$.
\end{itemize}
...
\item The absolute value of the term $\alpha = -\dfrac{\lambda_2}{\tau(\lambda_1 - \lambda_2)}$ is a characteristic frequency for the system. It delineates the low frequency domain in which approximate expressions help decompose the transfer functions in gain and delay components. In the spectral domain, $\lambda_{1}$ and $\lambda_{2}$ do appear as information propagation speeds while  $\tau\lambda_{1}$ acts as a characteristic damping distance.
\end{itemize}
