...
$
\phi_{21}(x,s) = 
\frac{\frac{\lambda_{1}}{\lambda_{2}} \alpha}
{s + \alpha}
e^{-\frac{x}{\lambda_2}s}
	\left(
...
We have $\frac{1}{\lambda_{1}}\left(-\alpha+\frac{1}{\tau}\right)=\frac{1}{\tau\left(\lambda_{1} - \lambda_{2}\right)}=\frac{-\alpha}{\lambda_{2}}$. It could appear at first sight that $-\alpha$, $>0$, is a singularity of the transfer functions and therefore the system is not bounded-input/bounded-output stable. However, a Taylor expansion about $-\alpha$ shows that numerators and denominators cancel each other out for $s \rightarrow -\alpha$ and $-\alpha$ is not a pole of any transfer function. This is enough to show that the output remains bounded for a given value of $x$. We will show below that a conic region of the $\left[0,T\right] \times \left[0,L\right]$ domain features exponential growth in free-flow regime. This arises when changing $t$ and $x$ simultaneously and complements the conclusion formulated from the spectral domain study (where $t$ varies and $x$ remains constant).

\subsubsection{Low frequency approximation in free-flow regime}
Analyzing the expressions above becomes easier when approximating them for $\left|s\right|\ll\left|\alpha\right|$. This is sensible concretely as one expects traffic related phenomena to occur at a low frequency. We find the following approximate expressions for the transfer functions above:
\begin{subequations}
\begin{align}
\psi_{11}(x,s) &=
	e^{-\frac{xs}{\lambda_{2}}}
...
\begin{itemize}
\item In $\psi_{11}$, $e^{-\frac{xs}{\lambda_{1}}}$ is a distributed delay with propagation speed $\lambda_{1}$ and $e^{-\frac{x}{\tau\lambda_{1}}}$ a distributed gain with characteristic distance $\tau\lambda_{1}$.
\item In $\psi_{12}$ and $\psi_{21}$ we can notice the combination of a distributed delay with  characteristic speed $\lambda_{2}$ and a distributed gain with $\tau\lambda_{1}$ as a characteristic distance.
...
The Bode plots for the physical variables are displayed in Figures \ref{fig:Magn_spatial_physx} and \ref{fig:Magn_phase_physx}.
For the Riemann invariants only $\phi_{21}(x,s)$ and $\phi_{22}(x,s)$ are represented in Figure \ref{fig:Magn_spatial_diag} and Figure \ref{fig:Magn_phase_diag} ($\phi_{11}(x,s)$ and $\phi_{12}(x,s)$ are only delay functions).
...
\tabularnewline
\end{tabular}
\caption{Spatial magnitude Bode plots for Riemann invariants\label{fig:Magn_spatial_diag}}
\end{figure}
...
\tabularnewline
\end{tabular}
\caption{Spatial magnitude Bode plots for physical variables\label{fig:Magn_spatial_physx}}
\end{figure}
...
\alpha\frac{\lambda_{1}}{\lambda_{2}}
	\left(
		e^{-\frac{x}{\lambda_{1}}\left(s+\frac{1}{\tau}\right)}
		-
		e^{-\frac{L}{\lambda_{1}\tau}}
		e^{-\frac{s}{\lambda_{2}}\left(x-L \left(1 - \frac{\lambda_{2}}{\lambda_{1}} \right)\right)}
	\right)
\frac{1}{s+\alpha}, \\
...
e^{-\frac{sx}{\lambda_{2}}}
	\left(
		e^{-\frac{x\left(s+\alpha\right)}{\lambda_{1}\tau}}
		-
		e^{-\frac{L}{\lambda_{1}\tau}}
		e^{\frac{sL}{\lambda_{2}}
			\left(1 - \frac{\lambda_{1}}{\lambda_{2}}\right)
		}
	\right)
\simeq \\
...
e^{-\frac{sx}{\lambda_{2}}}
	...
	\right)
$
is the combination of a gain, a distributed delay with propagation speed $\lambda_{2}$, and a sum of gain and delay components. The component $e^{\frac{sL}{\lambda_{2}}\left(1 - \frac{\lambda_{1}}{\lambda_{2}}\right)}$ seems to correspond to an anticipation term. This is to be expected given the upstream orientation of the characteristic lines of $\xi_{2}$. Indeed, the information contained in $\xi_{1}\left(0,s\right)$ flows backwards along the characteristic lines of $\xi_{2}$. The resulting bode plot is presented below in Figure \ref{fig:Magn_phase_physx_congested}.

\subsubsection{Transfer functions for physical variables in congested regime}
In congested regime, the boundary conditions used to control the system are $\hat{xi_{1}}\left(0,\cdot\right)$ and $\hat{xi_{2}}\left(0,\cdot\right)$. By linearity of the Laplace transform 
$\hat{\xi_{1}\left(0,s\right)} = 
\frac{
	\rho^{*}\lambda_{2}
}{
...
\frac{\rho^{*}s}{\lambda_{1} - \lambda_{2}}
\frac{
	e^{-\frac{x}{\tau\lambda_{1}}}
	e^{-\frac{s}{\lambda_{1}}
		\left(
...
We use the same fundamental diagram as in the free-flow case. However the linearization point, $\rho^* = 0.08$ veh/m, corresponds to the congested region of the Greeshields diagram.
...
\tabularnewline
\end{tabular}
\caption{Spatial magnitude Bode plots for Riemann invariants\label{fig:Magn_spatial_diag_congested}}
\end{figure}
...
\item The absolute value of the term $\alpha = -\dfrac{\lambda_2}{\tau(\lambda_1 - \lambda_2)}$ is a characteristic frequency for the system. It delineates the low frequency domain in which approximate expressions help decompose the transfer functions in gain and delay components. In the spectral domain, $\lambda_{1}$ and $\lambda_{2}$ do appear as information propagation speed. In several gain elements, $\tau\lambda_{1}$ and $\tau\lambda_{2}$ act as characteristic damping distances.
\end{itemize}
