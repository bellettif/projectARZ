...
\begin{abstract}
This article develops a linearized and diagonilazed version of the Aw-Rascle-Zhang model for freeway traffic modeling. We will focus on flow and speed macroscopic second-order models as opposed to microscopic and mesoscopic frameworks and leverage the simplifications brought by the linear approximation thanks to a spectral analysis of the system. An equivalent of the Froude number in hydrodynamics is designed that enables quick analysis of the nature of vehicle traffic flow. We prove that our linearization about an equilibrium is stable for congested regimes and unstable otherwise. Empirical NGSIM data corresponding to congested traffic trajectories is used so as to confront the linearized model's predictions to actual macroscopic behavior of traffic jam. The model achieves very good accuracy with a relative error of less than $20\%$ for speed and flow. It replicates in particular the propagation of boundary conditions' oscillations into the interior resolution domain of the PDE under study.
\end{abstract}
