...
\begin{itemize}
\item The TFN delineates two regimes, congested, for $F > 1$, corresponding 
\end{itemize}


\section{Conclusion}

As the full nonlinear ARZ equations are difficult to analyze, no closed form solutions are available. The linearized equations, are a stepping stone for the spectral methods presented here, allowing for elegantly simple yet powerful analysis tools, with the caveat that this holds only for local solutions. Nevertheless, the linearized model is able to capture important realistic features which first order models cannot. 

%With the linearized ARZ model, we were also able to define an interesting quantity. We found the eigenvalues of the linearized system to be constant, prompting the definition of the Traffic Froude Number, $F$, which indicates the flow regime of the road section under consideration. Note that the Froude number inequalities delineating the supercritical and subcritical regimes in hydronamics are reversed for our definition of the TFN. 

The spectral form also lead to interesting conclusions about the stability of the linearized system. The time domain responses given in Appendix A show that the system is unstable when $\lambda_2 > 0$ and stable when $\lambda_2 < 0$. That is, in the free-flow regime, $F < 1$, oscillations increase and the system leaves the linear regime, while in the congested regime, $F>1$, oscillations decrease and the system remains in the linear regime. Thus, the TFN is also an indication of linear stability. 
...
Future work will focus on controller design based on the spectral framework presented here. 
