...
Fourier and Laplace transforms are not the only spectral operators useful for studying traffic. Approaches such as \cite{Zheng2011} attempt to model spectral features of traffic using wavelets to account for turbulent phenomena on various time scales. The method presented here differs in that it is driven mostly by macroscopic physics rather than data analysis. Physics driven spectral analysis has for example been successful in \cite{fourierGalerkin} for data assimilation. A finite difference scheme was used in that case, we will use a different resolution method the equations of our model.
