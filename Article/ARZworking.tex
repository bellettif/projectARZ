...
\item In $\psi_{12}$ and $\psi_{21}$ we can notice the combination of a distributed delay with  characteristic speed $\lambda_{2}$ and a distributed gain with $\tau\lambda_{1}$ as a characteristic distance. It is also remarkable that $\widetilde{q}(x,s)$ appears as the result of a derivator applied to $\widetilde{v}(0,s)$.
...
A general comment can be made on transfer functions featuring $1 - e^{-\frac{x}{\lambda_{1} \tau \alpha} \left(s + \alpha \right)}$ as a factor, that is to say $\phi_{21}$, $\psi_{12}$, and $\psi_{21}$. As one can observe in the corresponding Bode plots, the value of the log-gain in high frequency tends to oscillate very sharply. Indeed, with $s = jw$,
$
\left| 
...
\item We can draw the same conclusions for $\theta_{21}$ although a derivator relates the output flux to the input speed.
...
\item The absolute value of the term $\alpha = -\dfrac{\lambda_2}{\tau(\lambda_1 - \lambda_2)}$ is a characteristic frequency for the system. It delineates the low frequency domain in which approximate expressions help decompose the transfer functions in gain and delay components. In the spectral domain, $\lambda_{2}$ appears as the information propagation speed of all distributed delay elements while  $\tau\lambda_{1}$ acts as the characteristic damping distance of all distributed gain elements.
\item Low frequency analysis also highlighted that transfer functions between physical variables in free-flow and congested regimes are structurally similar (for $\left|s\right|\ll\left|\alpha\right|$). The only dissimilarities that arise in the approximate expressions are the result of the different locations of the boundary conditions that have been chosen in free-flow and congested regimes respectively.
\end{itemize}
